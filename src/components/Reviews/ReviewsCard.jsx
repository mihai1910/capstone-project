...
export const ReviewsCard = (props) => {
    return(
        <article className="flex flex-col bg-white-primary w-30 lg:w-80 h-full px-2 lg:px-6 py-1 lg:py-4 mx-1.5 lg:mx-2 rounded-lg">
            ...
            <h2 className='text-black mt-2 mb-0.5 lg:mt-4 lg:mb-1 text-mobile-lg/3 lg:text-[2.125rem]/7 '>{props.title}</h2>
            <h3 className='text-[0.51rem] lg:text-md mb-2 lg:mb-4'>{props.description}</h3>
            <div className='flex mt-auto'>
                ...
                <div className='flex flex-col ml-2 md:ml-4 mt-0.5 lg:mt-1'>
                    <h3 className='text-[0.5rem] lg:text-md/3'>{props.name}</h3>
                    <h4 className='text-[0.5rem] lg:text-md opacity-40'>{props.date}</h4>
                </div>
            </div>
        </article>
    )
